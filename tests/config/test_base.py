...
def test_name_setter_raises_error():
    ...
    assert (
        "Trying to change name property to value New Name but you cannot change a property name after instantiation."
...
def test_value_setter_raises_error():
    ...
    assert (
        "Trying to change value to New Value but you cannot change a property value after instantiation."
        in str(exc_info.value)
    )
...
def test_type_is_valid_not_implemented():
    ...
    property_instance = IncompleteProperty(
        _name="Property Name", _value="Property Value"
    )
