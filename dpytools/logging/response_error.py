...
def map_error_json_to_object(error_dict: Dict):
    ...
    if "Cause" and "Code" and "Description" in error_dict:
        ...
        response_error = DatasetResponseError(cause=cause, error_code=error_code, description=description)
        return response_error
    
    else:
        # If the error from the response is not formatted according to expected standards, then raise this error instead
        raise ValueError(f"Error dict does not contain expected error keys (Cause, Code, Description). Dictionary contents: {error_dict}")