...
class IntegerProperty(BaseProperty):
    ...
    def type_is_valid(self):
        ...
        try:
            ...
            raise Exception(
                f"Cannot cast {self.name} value {self.value} to integer."
            ) from err
    ...
    def secondary_validation(self):
        ...
        if self.min_val and int(self.value) < self.min_val:
            raise ValueError(
                f"Integer value for {self.name} is lower than allowed minimum."
            )

        if self.max_val and int(self.value) > self.max_val:
            raise ValueError(
                f"Integer value for {self.name} is higher than allowed maximum."
            )
